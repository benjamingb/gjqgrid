    ...
    function __construct($adapter)
    {
        if (!$adapter instanceof AdapterInterface) {
            throw new Exception\InvalidArgumentException(sprintf(
                    '%s requires that $adapter be an object implementing %s; received "%s"', __METHOD__, __NAMESPACE__ . '\AdapterInterface', (is_object($adapter) ? get_class($adapter) : gettype($adapter))
            ));
        }
